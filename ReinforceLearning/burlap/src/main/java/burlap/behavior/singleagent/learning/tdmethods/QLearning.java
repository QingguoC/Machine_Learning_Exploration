...
public class QLearning extends MDPSolver implements QProvider, LearningAgent, Planner{
	...
	//track maxQ change in last N episodes
	protected double[]		trackLastNmaxQChange;
	public void setTrackLastNmaxQChange(double[] trackLastNmaxQChange) {
		this.trackLastNmaxQChange = trackLastNmaxQChange;
		for (int i = 0; i < trackLastNmaxQChange.length; i++){
			trackLastNmaxQChange[i] = Double.POSITIVE_INFINITY;
		}
	}
	//track rewards change in last N episodes
	protected double[]		trackLastNRewards;
	public void setTrackLastNRewardsChange(double[] trackLastNRewards) {
		this.trackLastNRewards = trackLastNRewards;
		for (int i = 0; i < trackLastNRewards.length; i++){
			this.trackLastNRewards[i] = Double.NEGATIVE_INFINITY;
		}
	}
	protected double maxRewardsChangeForPlanningTermination;
	public void setMaxRewardsChangeForPlanningTerminaiton(double m){
		if(m > 0.){
			this.maxRewardsChangeForPlanningTermination = m;
		}
		else{
			this.maxRewardsChangeForPlanningTermination = 0.;
		}
	}
	public double meanOfLastNmaxQChange(){
		double m = 0;
		for (int i = 0; i < trackLastNmaxQChange.length; i++){
			m += trackLastNmaxQChange[i];
		}
		return m/trackLastNmaxQChange.length;
	}
	...
	//get number of episodes when converge
	protected  int numEpisodes = 0;
	...
	public GreedyQPolicy planFromState(State initialState) {
		...
		int eCount = 0;
		int i = 0;
		double meanMaxQChange = Double.POSITIVE_INFINITY;
		do{
			this.runLearningEpisode(env, this.maxEpisodeSize);
			eCount++;

			i = eCount % this.trackLastNmaxQChange.length;
			this.trackLastNmaxQChange[i] = maxQChangeInLastEpisode;
			meanMaxQChange = this.meanOfLastNmaxQChange();
//			System.out.println("numEpisodesForPlanning-" + numEpisodesForPlanning + " maxQChangeInLastEpisode-" +
//					maxQChangeInLastEpisode + " maxQChangeForPlanningTermination-" + maxQChangeForPlanningTermination + " meanMaxQChange-" +meanMaxQChange );
			env.resetEnvironment();
		}while(eCount < numEpisodesForPlanning && meanMaxQChange > maxQChangeForPlanningTermination);

		this.numEpisodes = eCount;
